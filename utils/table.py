...
def addtable(table, i,k, r, action, data, word, j, tmatch):
    return "table_add MyIngress." + table + "_" + str(i) + str(k) + "_r" + str(r) + " " + action + "_" + data + str(word) + " " + str(j) + " => " + tmatch[j]


lutd = [td0, td1, td2, td3]
lute = [te0, te1, te2, te3]

#Table aes_sbox - GEN_LUT_SBOX - GENERATE_ALL_TABLE_SBOX
for n in range (12):
    for rw in range(5):
        for w in range(4):
            for m in range (256):
                print(addtable("aes_sbox", rw, w, n, "merge_sbox_to", "w", w, m, sbox))

#Table aes_sbox 256 - GEN_LUT_SBOX - GENERATE_TABLE_SBOX_256
for n in range (6):
    for w in range(4):
        for m in range (256):
            print(addtable("aes_sbox", 5, w, n, "merge_sbox_to", "t", w, m, sbox))

#Table aes_sbox LAST round - GEN_LUT_SBOX - GENERATE_ALL_TABLE_LAST
for rw in range(5):
    for w in range(4):
        for m in range (256):
            print(addtable("aes_sbox", rw, w, 12, "merge_sbox_to", "w", w, m, sbox))

#Table aes_lut - UNGEN_LUT - GENERATE_ALL_UNTABLE_LUT_EXPAND
for n in range (13):
    for rw in range(4):
        for w in range(4):
            for m in range (256):
                print(addtable("aes_sbox_lut_exp", rw, w, n, "merge_inv_to", "ti", w, m, lutd[w]))

#Encrypt/Decrypt
#Table UNTABLE_LUT - UNGEN_LUT - GENERATE_ALL_UNTABLE_LUT
for n in range (1,14):
    for rw in range(4):
        for w in range(4):
            for m in range (256):
                print(addtable("aes_invsbox_lut", rw, w, n, "unmerge_to", "t", rw, m, lutd[w]))

#Table UNTABLE_LUT LAST - GEN_LUT_INVSBOX
for rw in range(4):
    for w in range(4):
        for m in range (256):
            print(addtable("aes_invsbox_lut", rw, w, "LAST", "unmerge_to_t" + str(rw), "slice", w, m, sboxi))

#Table TABLE_LUT - GEN_LUT - GENERATE_ALL_TABLE_LUT
for n in range (1,14):
    for rw in range(4):
        for w in range(4):
            for m in range (256):
                print(addtable("aes_sbox_lut", rw, w, n, "merge_to", "t", rw, m, lute[w]))


#Table TABLE_LUT LAST - GEN_LUT_SBOX
for rw in range(4):
    for w in range(4):
        for m in range (256):
            print(addtable("aes_sbox_lut", rw, w, "LAST", "merge_to_t" + str(rw), "slice", w, m, sbox))
