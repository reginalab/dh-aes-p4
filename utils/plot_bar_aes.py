...
def format_plot(graph, ylabel, xlabel):
    ...
    leg = plt.legend(loc='upper left',
                     ...
                     shadow=False, borderpad=1, fontsize="small")
...
def plot_graphs(logexp):
    ...
    fig, ax = plt.subplots()
    rects1 = ax.bar(x - width/1.9, means_enc, width, label='Encryption time', yerr=std_enc, align='center',ecolor='black', capsize=10, color=['grey'])
    rects2 = ax.bar(x + width/1.9, means_dec, width, label='Decryption time', yerr=std_dec, align='center',ecolor='black', capsize=10, color=['silver'])
